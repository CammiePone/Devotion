...
public class AmethystAltarBlockEntity extends BlockEntity implements Inventory {
	...
	public AmethystAltarBlockEntity(BlockPos pos, BlockState state) {
		super(DevotionBlockEntities.AMETHYST_ALTAR, pos, state);
	}
		...
		if(world != null) {
			...
			for(Map.Entry<BlockPos, BlockState> entry : DevotionHelper.getStructureMap(world).entrySet()) {
				if(entry.getValue().getFluidState().isIn(FluidTags.WATER)) {
					BlockPos.Mutable waterPos = entry.getKey().mutableCopy().move(pos).move(DevotionHelper.getAltarOffset(world));
	...
	public void checkMultiblock() {
		if(world != null && !world.isClient()) {
			...
			HashMap<BlockPos, BlockState> template = DevotionHelper.getStructureMap(world);

			for(Map.Entry<BlockPos, BlockState> entry : template.entrySet()) {
				BlockPos.Mutable pos = getPos().mutableCopy().move(DevotionHelper.getAltarOffset(world)).move(entry.getKey());
				BlockState state = world.getBlockState(pos);

				if(DevotionHelper.isValidAltarBlock(state))
