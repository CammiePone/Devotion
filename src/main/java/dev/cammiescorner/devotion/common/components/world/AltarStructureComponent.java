package dev.cammiescorner.devotion.common.components.world;
...
public class AltarStructureComponent implements AutoSyncedComponent {
	...
	public void constructStructureMap() {
		if(world instanceof ServerWorld serverWorld) {
			structureMap.clear();
			Optional<Structure> optional = serverWorld.getStructureManager().getStructure(Devotion.id("altar"));
			...
			if(optional.isPresent()) {
				...
				for(Structure.StructureBlockInfo info : infoList)
					if(DevotionHelper.isValidAltarBlock(info.state))
						structureMap.put(info.pos.subtract(pos), info.state);

				DevotionComponents.ALTAR_STRUCTURE_COMPONENT.sync(world);
			}
		}
	}
}
