...
public record BookTab(ItemStack icon, int order) {
	public static final Codec<Holder<BookTab>> CODEC = RegistryFixedCodec.create(DevotionRegistryKeys.BOOK_TAB);
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, Holder<BookTab>> STREAM_CODEC = ByteBufCodecs.holderRegistry(DevotionRegistryKeys.BOOK_TAB);

	public static BookTab get(ResourceLocation id, RegistryAccess access) {
		return access.registry(DevotionRegistryKeys.BOOK_TAB).orElseThrow().get(id);
	}

	public ResourceLocation getId(RegistryAccess access) {
		return access.registry(DevotionRegistryKeys.BOOK_TAB).orElseThrow().getKey(this);
	}
}
